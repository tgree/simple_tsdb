...
class tmock_test
{
    TMOCK_TEST(test_create_root)
    {
        ...
        {
            tsdb::root root(".",true);
            TABORT("Expected exception because root doesn't exist!");
        }
        ...
        TASSERT_EQUIV(fs_root->subdirs.size(),2UL);
        TASSERT_EQUIV(fs_root->files.size(),3UL);
        ...
        {
            tsdb::create_root(".",c);
            TABORT("Expected exception because root exists!");
        }
        catch (const tsdb::init_io_error_exception& e)
        {
            TASSERT_EQUIV(e.errnov,EEXIST);
        }

        tsdb::root root(".",true);
        TASSERT_EQUIV(root.config.chunk_size,2UL*1024UL*1024UL);
        TASSERT_EQUIV(root.config.wal_max_entries,10240UL);
        TASSERT_EQUIV(root.config.write_throttle_ns,1000000000UL);
    ...
    TMOCK_TEST(test_chunk_sizes)
    {
        ...
        {
            tsdb::create_root(".",c);
            TABORT("Expected invalid chunk size exception!");
        }
        ...
        for (uint64_t cs : chunk_sizes)
        {
            ...
            tsdb::create_root(".",c);
            tsdb::root root(".",true);
            TASSERT_EQUIV(root.config.chunk_size,cs);
        }
    }
    ...
    TMOCK_TEST(test_users)
    {
        ...
        assert_tree_fsynced(fs_root);
        TASSERT_EQUIV(fs_root->files["passwd"]->data_as_string(),
        ...
        try
        {
            ...
            TABORT("Expected duplicate user exception!");
        }
        ...
        try
        {
            ...
            TABORT("Expected no-such-user exception!");
        }
    ...
    TMOCK_TEST(test_create_databases)
    {
        ...
        TASSERT_EQUIV(db_list.size(),3UL);
        TASSERT_EQUIV(db_list[0],"db1");
        TASSERT_EQUIV(db_list[1],"db2");
        TASSERT_EQUIV(db_list[2],"db3");
    }
};
